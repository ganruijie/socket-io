class Channel {
  ...
  init () {
    ...
    // 新用户注册
    this.socket.on(this.cxt.eventKeys.client.registerUser, (id, name) => {
      console.log(`${id}-${name}--${self.id}`)
      self.cxt.createUserById(id, name, self.id)
    })
    // 发送消息
    this.socket.on(this.cxt.eventKeys.client.newMsg, (msg) => {
      self.notifyMsg(msg)
      console.log(msg)
      self.cxt.addMsg(msg)
    })
    this.socket.on(this.cxt.eventKeys.client.closeConn, () => {
